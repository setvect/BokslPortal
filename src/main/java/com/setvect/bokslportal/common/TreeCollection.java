...
public class TreeCollection<OBJ extends TreeItem<?>> {
  ...
  /**
   * @param baseRootCategoryID 트리 시작 지점
   ...
   * @param rootCategory    시작되는 카테고리
   * @param rootSave        루트 카테고리 포함 여부
   * @param excludeCategory 제외 할 메뉴(하위 그룹까지 다 제외)
   * @return 정렬된 카테고리 배열
   */
   ...
   *
   * @param formCategoryID 루트로 부터 formCategoryID카테고리 까지 경로를 구함
   * @return 카테고리 경로
   */
  public List<OBJ> getPath(Object formCategoryID) {
    ...
    OBJ[] descCategory = saveCategory.toArray(_obj_empty_array);
  ...
  /**
   * @param currentCategoryID 현재 카테고리 아이디
   * @param saveCategory      재귀 호출을 통해 카테고리 정보를 담는 변수
   */
  ...
  /**
   * @param rootCategory    시작 카테고리
   * @param depth           현재 진행 카테고리 깊이
   * @param modifyLevel     ture 깊이 정보 변경, false 변경하지 않음
   * @param rootSave        루트 카테고리 포함 여부
   * @param excludeCategory 제외 할 메뉴(하위 그룹까지 다 제외)
   * @return 정렬된 카테고리 배열
   */
  private List<OBJ> getTree(Object rootCategory, int depth, boolean modifyLevel, boolean rootSave,
    ...
    OBJ[] cat = map.toArray(_obj_empty_array);
    if (rootSave) {
