...
public class StringEncrypt {
  ...
  public static String encodeJ(String pm_sStr, String pm_sKey) {
    ...
    for (i = 0; i < s; i += CUT_STRING_LEN) {
      if (i + CUT_STRING_LEN < s) {
        lm_sbT.append(lm_sValue, i, i + CUT_STRING_LEN);
      } else {
        lm_sbT.append(lm_sValue, i, s);
      }
  ...
  /**
   * @param pm_sEncode    Hex로 인코딩 된 문자
   * @param pm_sKeyEncode Hex로 인코딩 된 키
   * @return 변환된 문자열 (Hex값)
   */
