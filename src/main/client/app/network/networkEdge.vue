<template>
  <b-modal ref="edgeForm" title="연결선 추가" @ok.prevent="confirm()">
    <div>
      <b-form autocomplete="off" @submit.stop.prevent>
        <b-form-group label="레이블" label-for="input-label">
          <b-form-input v-model="edge.label" name="label" id="input-label" data-vv-as="레이블 " :state="validateState('label')" v-validate="{ max:30}" />
        ...
        <b-form-group label="색" id="input-color">
          <b-form-radio-group v-model="edge.color">
